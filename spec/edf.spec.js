...
describe('Edf', () => {
  def('edfHeaderBuffer', () => Buffer.from('0       MCH-0234567 F 16-SEP-1987 Haagse_Harry                                          Startdate 16-SEP-1987 PSG-1234/1987 NN Telemetry03                              16.09.8720.35.00768     Reserved field of 44 characters             2880    30      2   EEG Fpz-Cz      Temp rectal     AgAgCl cup electrodes                                                           Rectal thermistor                                                               uV      degC    -440    34.4    510     40.2    -2048   -2048   2047    2047    HP:0.1Hz LP:75Hz N:50Hz                                                         LP:0.1Hz (first order)                                                          15000   3       Reserved for EEG signal        Reserved for Body temperature    ', 'ascii'))

  def('edfDataBuffer', () => fs.readFileSync(__dirname + '/resources/20190828_032858_SAD.edf'))

  describe('.fromBuffer', () => {
    test('returns an Edf object', async () => {
      const edf = await Edf.fromBuffer(get('edfHeaderBuffer'))
    ...
    describe('returns an Edf object that', () => {
      ...
      describe('when the start date does not appear in the record ID field', () => {
        def('edfHeaderBuffer', () => Buffer.from(get('edfHeaderBuffer').toString('ascii').replace('Startdate 16-SEP-1987', '                     '), 'ascii'))
        ...
        describe('and the year is 2020', () => {
          def('edfHeaderBuffer', () => {
            const modifiedHeader = get('edfHeaderBuffer')
              .toString('ascii')
      ...
      test('contains the measurements of each signal', async () => {
        const edf = await Edf.fromBuffer(get('edfDataBuffer'))
