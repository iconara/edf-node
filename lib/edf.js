...
function parseSignalValues (buffer, sampleCount) {
  const values = []
  for (let i = 0; i < sampleCount; i++) {
    values.push(buffer.readInt16LE(i * 2))
  }
  return Int16Array.from(values)
}

Edf.fromBuffer = async function (buffer) {
  ...
  if (buffer.length > header.headerSize) {
    ...
    for (const signal of signals) {
      signal.data = parseSignalValues(buffer.subarray(offset), signal.sampleCount)
