...
class Edf {
  ...
  static async fromBuffer (buffer) {
    ...
    if (buffer.length > header.headerSize) {
      for (const signal of signals) {
        signal.data = new Int16Array(signal.sampleCount * header.recordCount)
      }
      let offset = header.headerSize
      for (let recordIndex = 0; recordIndex < header.recordCount; recordIndex++) {
        for (const signal of signals) {
          const recordValues = Int16Array.from(signalValues(buffer.subarray(offset), signal.sampleCount))
          signal.data.set(recordValues, recordIndex * signal.sampleCount)
    ...
    const metadata = {
      ...
      duration: header.recordDuration * header.recordCount * 1000,
    }
  ...
  toTable () {
    const vectors = this._signals.map((s) => arrow.Int16Vector.from(s.data))
    const names = this._signals.map((s) => s.label)
    const table = arrow.Table.new(vectors, names)
    table.schema.metadata.set('startDateTime', this.startDateTime)
    table.schema.metadata.set('duration', this.duration)
    for (let i = 0; i < this._signals.length; i++) {
      for (const signalHeaderField of SIGNAL_HEADER_FIELDS) {
        table.schema.fields[i].metadata.set(signalHeaderField.name, this._signals[i][signalHeaderField.name])
      }
    }
    return table
  }
