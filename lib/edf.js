...
function* timestamps (startTimestamp, recordDuration, recordCount, sampleCount) {
  const endTimestamp = startTimestamp + recordDuration * recordCount
  const sampleDuration = recordDuration/sampleCount
  for (let ts = startTimestamp; ts < endTimestamp; ts += sampleDuration) {
    yield ts
  }
}
...
class Edf {
  static async fromBuffer (buffer) {
    ...
    const signalMeasurements = sampleCounts.map((sampleCount) => new Int16Array(sampleCount * header.recordCount))
    if (buffer.length > header.headerSize) {
      ...
      for (let recordIndex = 0; recordIndex < header.recordCount; recordIndex++) {
        for (let signalIndex = 0; signalIndex < signalMetadata.length; signalIndex++) {
          ...
          const signalData = signalMeasurements[signalIndex]
          const recordValues = Int16Array.from(signalValues(buffer.subarray(offset), sampleCount))
          signalData.set(recordValues, recordIndex * sampleCount)
    ...
    const signalColumns = signalMetadata
      .map((metadata, index) => {
        ...
        const vector = arrow.Int16Vector.from(signalMeasurements[index])
        return arrow.Column.new(field, vector)
      })
      .filter((column) => column.name !== 'Crc16')
    const startTimestamp = parseDateTime(header)
    const timestampGenerator = timestamps(startTimestamp, recordDuration, header.recordCount, sampleCounts[0])
    const timestampField = arrow.Field.new('timestamp', new arrow.TimestampMillisecond(), false)
    const timestampVectors = arrow.Builder.throughIterable({type: new arrow.TimestampMillisecond()})(timestampGenerator)
    const timestampColumn = arrow.Column.new(timestampField, ...timestampVectors)
    const table = arrow.Table.new([timestampColumn, ...signalColumns])
    table.schema.metadata.set('startTimestamp', startTimestamp)
