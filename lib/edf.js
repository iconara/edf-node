...
function parseSignals (buffer, header, signalMetadata) {
  ...
  return signalMetadata.map((metadata, signalIndex) => {
...
class Edf {
  static fromBuffer (buffer) {
    ...
    const signalColumns = parseSignals(buffer, header, signalMetadata)
    const table = arrow.Table.new(signalColumns.filter(({name}) => name !== 'Crc16'))
