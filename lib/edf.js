...
class Edf {
  static async fromBuffer (buffer) {
    ...
    const signalMeasurements = signalMetadata.map((metadata) => new Int16Array(metadata.get('sampleCount') * header.recordCount))
    if (buffer.length > header.headerSize) {
      ...
      for (let recordIndex = 0; recordIndex < header.recordCount; recordIndex++) {
        for (let signalIndex = 0; signalIndex < signalMetadata.length; signalIndex++) {
          const sampleCount = signalMetadata[signalIndex].get('sampleCount')
    ...
    const signalColumns = signalMetadata.map((metadata, index) => {
    ...
    table.schema.metadata.set('startDateTime', parseDateTime(header))
  ...
  get startDateTime () {
    return this._table.schema.metadata.get('startDateTime')
  }
